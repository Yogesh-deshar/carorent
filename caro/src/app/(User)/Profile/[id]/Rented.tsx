...
const Rented = ({ userId }: { userId: string }) => {
    ...
    const fetchBookings = async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/bookings/fetchbooking/user/${userId}`,
        );
  ...
  if (loading) {
    return <p className="text-gray-600 mt-4">Loading your rented cars...</p>;
  }
